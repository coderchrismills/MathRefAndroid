...
public class MainActivity extends FragmentActivity implements OnHeadlineSelectedListener {
	...
	public static void ShowUpSellDialog(String title, 
										String message, 
										String yesButton, 
										String noButton, 
										DialogInterface.OnClickListener yesPressed, 
										DialogInterface.OnClickListener noPressed) 
	{
		AlertDialog show = new AlertDialog.Builder(GlobalHelpers.mDialogContext)
				.setTitle(title)
				.setTitle(message)
				.setCancelable(false)
				.setPositiveButton(yesButton, yesPressed)
				.setNegativeButton(noButton, noPressed).show();     
	}
	...
	private void HandleSelection(String section, String subsection, int equation, boolean frompress, boolean addback) {
		...
		if(section.equalsIgnoreCase("about")) {
			...
			mMasterViewFragment = new ArticleFragment();
			((ArticleFragment)mMasterViewFragment).SetArticleView(ArticleFragment.ArticleViews.About);
		}
		...
		// equation selected
			...
			mMasterViewFragment = new ArticleFragment();
			((ArticleFragment)mMasterViewFragment).SetArticleView(ArticleFragment.ArticleViews.Equation);
			Bundle args = new Bundle();
		...
		// Sub sections
			...
			if(subsection.equalsIgnoreCase("periodic table")) {
				...
				mMasterViewFragment = new ArticleFragment();
				((ArticleFragment)mMasterViewFragment).SetArticleView(ArticleFragment.ArticleViews.Periodic);
			}
			else {
				mMasterViewFragment = new LeafViewController();
				Bundle args = new Bundle();
	...
	private Fragment getFragmentWithFragmentId(String subsection) {
		...
		if(subsection.equalsIgnoreCase("two line solver")) {
			((ArticleFragment)frag).SetArticleView(ArticleFragment.ArticleViews.TwoLine);
		}
		else if(subsection.equalsIgnoreCase("quadratic solver")) {
			((ArticleFragment)frag).SetArticleView(ArticleFragment.ArticleViews.Quadratic);
		}
		else if(subsection.equalsIgnoreCase("exponential solver")) {
			((ArticleFragment)frag).SetArticleView(ArticleFragment.ArticleViews.Exponential);
		}
		else if(subsection.equalsIgnoreCase("complex solver")) {
			((ArticleFragment)frag).SetArticleView(ArticleFragment.ArticleViews.Complex);
		}
		else if(subsection.equalsIgnoreCase("triangle solver")) {
			((ArticleFragment)frag).SetArticleView(ArticleFragment.ArticleViews.Triangle);
		}
		else if(subsection.equalsIgnoreCase("circle solver")) {
			((ArticleFragment)frag).SetArticleView(ArticleFragment.ArticleViews.Circle);
		}
		else if(subsection.equalsIgnoreCase("angle converter")) {
			((ArticleFragment)frag).SetArticleView(ArticleFragment.ArticleViews.AngleConverter);
		}
		else if(subsection.equalsIgnoreCase("inverse solver")) {
			((ArticleFragment)frag).SetArticleView(ArticleFragment.ArticleViews.InverseSolver);
		}
		else if(subsection.equalsIgnoreCase("reference angle solver")) {
			((ArticleFragment)frag).SetArticleView(ArticleFragment.ArticleViews.RefAngle);
		}
		else if(subsection.equalsIgnoreCase("z solver")) {
			((ArticleFragment)frag).SetArticleView(ArticleFragment.ArticleViews.ZSolver);
		}
		else if(subsection.equalsIgnoreCase("unit converter")) {
			((ArticleFragment)frag).SetArticleView(ArticleFragment.ArticleViews.UnitConverter);
		}
		return frag;
	}
